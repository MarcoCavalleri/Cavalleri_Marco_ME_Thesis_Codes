...
D_1, D_2, D_s = 0.5, 10, 1

mu0_1, mu0_2, mu0_s = 0, 0, 0

s = 1
s_1, s_2, s_3, s_4 = s, s, s, s

# L and K ate not defined as matrices but it will be used their entries directly in the equations
chi = 0
k1 = 0.1
k2 = 0.05
mu_Y1, mu_Y2, mu_Y3, mu_Y4 = log(2), log(5), log(0.01), log(0.1)
...
perturbation = 0.05 # <-- Square root of the variance
...
# Chemical potentials
mu_1 = f"({mu0_1} + log(c_1) - {k1}*laplace(c_1) - {k2}*laplace(c_2))"
mu_2 = f"({mu0_2} + log(c_2) - {k1}*laplace(c_2) - {k2}*(laplace(c_1 + c_s)))"
mu_s = f"({mu0_s} + log(c_s) - {k1}*laplace(c_s) - {k2}*laplace(c_2))"

# Reaction currents
j_1 = f"{s_1}*(exp({mu_Y1})-exp({mu_1}))"
j_2 = f"{s_2}*(exp(2*({mu_1})+{mu_2})-exp(3*({mu_1})))"
j_3 = f"{s_3}*(exp({mu_1} + {mu_Y2})-exp({mu_2} + {mu_Y4}))"
j_4 = f"{s_4}*(exp({mu_1})-exp({mu_Y3}))"

# Chemical reaction terms
chem_term_1 = f"({j_1}+{j_2}-{j_3}-{j_4})"
chem_term_2 = f"(-{j_2}+{j_3})"
...
physics = f"model-0-fig2_{ic_type}_T={T}"
...
if ic_type == "previous":
    ...
    input_dir = r'results_fig2' # <-- Directory
    filenames = {
        'c_1': os.path.join(input_dir, 'c_1_fig2.txt'),
        'c_2': os.path.join(input_dir, 'c_2_fig2.txt'),
        'c_s': os.path.join(input_dir, 'c_s_fig2.txt'),
    }

    # Helper function to load concentration from file
    def load_concentration(filename, n_x, n_y):
        data = np.loadtxt(filename, comments='#')
        # The 3rd column holds the concentration data
        field_flat = data[:,2]
        # Reshape as grid (row-major ordering: Y is the slowest, X is the fastest)
...
# Assemble
state = FieldCollection([c_1, c_2, c_s])
...
output_dir = r"results_fig2" # <-- Possibly the same as input_dir
...
np.savetxt(os.path.join(output_dir, "c_1_fig2.txt"), c_1_data, header="x y c_1")
np.savetxt(os.path.join(output_dir, "c_2_fig2.txt"), c_2_data, header="x y c_2")
np.savetxt(os.path.join(output_dir, "c_s_fig2.txt"), c_s_data, header="x y c_s")
